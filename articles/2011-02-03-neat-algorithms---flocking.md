...
Heres the essence of the Coffeescript class modeling the boid. Each boid has a `location`, and a `velocity`, represented as Vector objects ([source](https://github.com/hornairs/blog/blob/master/assets/coffeescripts/flocking/vector.coffee)). Each frame calls the `step` method on each boid, which calculates an acceleration based on the 3 components. This acceleration is added to the velocity and limited so the boid can't go too fast. The new velocity is added to the location to actually move the boid.
...
I hope you found this informative! If you have any questions, comments, corrections, or know of any neat uses of flocking algorithms in the wild (baha!), please leave them below. 
